ennet <- function(Ex,Pt,Tm,Tf,delay=1,ntree=1000,shrinkage=0.001,bag.fraction=0.3,col.fraction=0.3,
  ...
  V <- foreach(i=1:N, .inorder=TRUE, .combine="cbind") %dopar% {
    ...
    model       <- ennet.train(
      X.train=E.all[F_sample,predictorsI],
      Y.train=E.all[T_sample,predictedI],
      M.train=ntree,
      nu=shrinkage,
      s_s=bag.fraction,
      s_f=col.fraction
