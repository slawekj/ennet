mart <- function(Ex,Pt,Tm,Tf,delay=1,ntree=1000,interaction.depth=1,
                 n.minobsinnode=1,shrinkage=0.001,bag.fraction=0.632,train.fraction=1,
  ...
  V <- foreach(i=1:N, .inorder=TRUE, .combine="cbind") %dopar% {
    ...
    result              <- rep(0,N);
    result[predictorsI] <- summary.gbm(model,n.trees=ntree,plotit=FALSE,order=FALSE)[,2];
    result
  }
